...
int krb_ap_rep_check(krb5_kdc_rep *packet, krb5_error *err, krb5_authenticator *cleartext, krb5_authenticator *authenticator)
{
	...
	*/
	...
	if (cleartext->cusec != authenticator->cusec)
		printf("%d", err->error = KRB_AP_ERR_MUT_FAIL);
	if (cleartext->subkey->contents != NULL)
		1;/* save cleartext.subkey for future use;*/
	if (cleartext->seq_number != NULL)
